...
void test_init(void)
{
    ...
    assert(core_file_init(&f, "test_read.txt", 16, 64) == SUCCESS);

    assert(core_file_get_filepath(&f) != NULL);

    assert(strcmp(core_string_get_data(core_file_get_filepath(&f)),
                  "test_read.txt") == 0);
...
void test_init_null_filepath(void)
{
    ...
    assert(core_file_init(&f, NULL, 16, 64) == STRING_CSTR_NULL_ERROR);
...
void test_read(void)
{
    ...
    assert(core_file_read(&f) == SUCCESS);
...
void test_read_line_content(void)
{
    ...
    string_t* first =
        (string_t*)core_vector_get(core_file_get_lines(&f), 0);
...
void test_write(void)
{
    ...
    assert(core_file_init(&f, "test_write.txt", 16, 64) == SUCCESS);

    core_file_read(&f);

    assert(core_file_write(&f) == SUCCESS);
...
void test_write_to_different_file(void)
{
    ...
    size_t line_count =
        core_vector_get_size(core_file_get_lines(&f));
    ...
    assert(core_vector_get_size(core_file_get_lines(&f)) ==
           line_count);
